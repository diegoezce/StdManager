...
export function Navbar() {
  ...
  const navItems: { label: string; href: string; roles: string[] }[] = [
    ...
    { label: 'Estudiantes', href: '/estudiantes', roles: ['corporate_client'] },
            ...
            <div className="hidden md:flex space-x-1 ml-10">
              {visibleItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="px-3 py-2 rounded-md text-sm font-medium text-slate-600 hover:text-indigo-700 hover:bg-slate-50 transition"
                >
                  {item.label}
                </Link>
              ))}
